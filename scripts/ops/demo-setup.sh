...
for c in curl jq docker; do
  command -v "$c" >/dev/null 2>&1 || err "缺少依赖: $c"
done
...
for i in $(seq 1 30); do
  ...
  [ "$i" -eq 30 ] && err "容器 30s 未就绪，请先部署（vanblog.sh install）"
  sleep 1
done
...
[ -n "$TOKEN" ] && [ "$TOKEN" != "null" ] || err "登录失败（账号/密码被改?）"
SITE_ID=$(curl -sf "$PB_URL/api/collections/site/records?perPage=1" \
  -H "Authorization: Bearer $TOKEN" | jq -r '.items[0].id')
[ -n "$SITE_ID" ] && [ "$SITE_ID" != "null" ] || err "未找到 site 记录"
curl -sf -X PATCH "$PB_URL/api/collections/site/records/$SITE_ID" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d "{\"allowedDomains\":[\"$DOMAIN\"]}" >/dev/null || err "设置 allowedDomains 失败"
ok "allowedDomains 已设置"

# ── 3. 灌示例文章 ──────────────────────────────────────
info "灌入 $SEED_COUNT 篇示例文章…"
dc exec -T vanblog vanblog seed --count "$SEED_COUNT" --dir=/pb_data || err "seed 失败"
ok "seed 完成"
