...
| 锁定卡加密提示 | 列表页锁定卡为裸卡(摘要被 API 遮蔽为空 → 无提示区) | SSR 为锁定文合成 `encryptedHtml` 提示(「该文章已加密…」+占位条)经 props 下发 | 有意可见分叉:匿名视角信息量更优;原版数据面不产提示文本,无泄漏等价问题(2026-09-11 parity 登记待复核是否对齐) |
