...
| 变量                           | 必填 | 默认值 | 说明                                                                                                     |
| ------------------------------ | ---- | ------ | -------------------------------------------------------------------------------------------------------- |
| `VANBLOG_ARTALK_UPSTREAM`      | 否   | （空） | 外部 Artalk 容器地址（如 `artalk:23366`）。仅表示外部上游地址，是否启用由 `site.commentsProvider` 决定。 |
| `VANBLOG_COMMENTS_SSO_ENABLED` | 否   | （空） | 设为 `1` 或 `true` 启用 Artalk SSO 免二次登录桥接（`/api/vanblog/comments-sso/*`）。默认关闭。           |
...
| `s3Config`                              | json | **已迁至 `site_secrets` 集合（admin-only）**：S3/对象存储配置（`{enabled,bucket,region,endpoint,accessKey,secret,forcePathStyle}`）。修改后自动同步到 pb settings，无需重启。**secret 明文存储**，见 [SECURITY](../../SECURITY.md)。`site` 集合公开可读，凭据类配置（`s3Config`/`syncConfig`/`syncRemote`/`outputConfig`）一律存 `site_secrets`：对 `site` 的写请求会由平台钩子自动把密钥字段移入 `site_secrets`，也可直接在 pb Admin UI 编辑 `site_secrets`。 |
