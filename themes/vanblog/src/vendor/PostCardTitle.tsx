 ...
 * → ./Viewer island;5) 评论计数 span 类名/初始 "0"/data-path 逐字保留,文章页由
 * CommentArtalk 按 Artalk stats 填充,首页保持静态 0(上游 WaLine 不挂首页,同)。
...
export function Title(props: {
  ...
  return (
    <div className="flex justify-center post-card-title ">
      {props.type != "about" ? (
        <a href={withBase(`/post/${props.id}`)} target={getTarget(newTab)} style={{width:"90%"}} title={props.title}>
          <div
...
export function SubTitle(props: {
  ...
  id: number | string;
  openArticleLinksInNewWindow: boolean;
  /** SEAM: 服务端注入的当前浏览计数(上游 PostViewer 自取 res.viewer) */
  views?: number;
}) {
  const iconSize = "16";
  const iconClass =
    ...
    <div className="text-center text-xs md:text-sm divide-x divide-gray-400 text-gray-400 dark:text-dark post-card-sub-title">
      ...
      {props.type != "about" && (
        <span className="inline-flex px-2 items-center group dark:group cursor-pointer">
          ...
          <a
            href={withBase(`/category/${encodeQuerystring(props.catelog)}`)}
      ...
      <span className="inline-flex px-2 items-center">
        ...
        <Viewer shouldAddViewer={props.type != "overview"} id={String(props.id)} initial={props.views ?? 0} />
      </span>
