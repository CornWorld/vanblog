/* UPSTREAM: packages/website/components/SearchCard/index.tsx@4b488500be8100b19772ec00d8315f343a6ac21e
 * SEAM: 数据层 — searchArticles → seams/search.ts(平台 /api/vanblog/search);
 * ArticleList/KeyCard 为本目录 vendor 件。⌘K/Esc 监听、500ms 防抖、状态文案、
 * 缩放进出动画全部上游原样。上游 fix → 对本文件 apply patch。
 */
import { useEffect, useMemo, useRef, useState } from "react";
// 搜索防抖:输入停 500ms 再发请求,新输入取消旧计时。
// (原用 react-use 的 useDebounce;CJS 包的 named export 在 Node ESM
// 命名空间导入下解析不出——三轮导入写法均无法在 SSR 运行时稳定,
// 组件仅此一处用该库,直接内联替掉依赖。)
...
export default function SearchCard(props: {
  ...
  return (
    <div
      ...
      onClick={(ev) => {
        if (innerRef.current) {
          if (!(innerRef.current as Node).contains(ev.target as Node)) {
            // 外
            document.body.style.overflow = "auto";
            props.setVisible(false);
          }
