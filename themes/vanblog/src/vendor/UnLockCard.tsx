...
import { unlockPost } from "./seams/unlock";

/** 上游 Loading/index.tsx 原样(children 类型按项目规则收窄)。 */
function Loading(props: { children: React.ReactNode; loading: boolean }) {
  if (props.loading) {
...
export default function UnLockCard(props: {
  ...
  const fetchArticle = async () => {
    try {
      const res = await unlockPost(String(props.id), value);
      if (!res.ok) {
        onError(res.message);
        return false;
      }
