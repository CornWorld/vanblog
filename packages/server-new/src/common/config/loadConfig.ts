...
if (process.env.VAN_BLOG_CONFIG_FILE) {
  ...
  rawConfigs = [path.resolve('/etc/van-blog/config.yaml'), path.resolve('./config.yaml'), path.resolve('./config.yml')];
}
...
console.log(yaml.parse(fs.readFileSync(path.resolve('./config.yaml')).toString()));
...
export const loadConfig = <T = unknown>(key: string, defaultValue?: T | (() => T)): T => {
  ...
  else if (typeof defaultValue === 'function') ret = { t: 'def_func', val: (defaultValue as () => T)() };
