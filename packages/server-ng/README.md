...
演示模式模块为 VanBlog 提供了完整的演示环境管理功能，支持数据快照和自动恢复机制。

#### 主要功能

- **数据快照**: 自动创建博客数据的完整快照，包括文章、草稿、分类、标签等
- **定时恢复**: 每 6 小时自动恢复到初始演示状态
- **手动恢复**: 提供 API 接口支持手动触发数据恢复
- **状态查询**: 查询演示模式状态和快照信息
- **权限控制**: 基于角色的访问控制，确保只有管理员可以操作

#### API 端点

- `GET /api/v2/demo/status` - 获取演示模式状态
- `POST /api/v2/demo/restore` - 手动恢复演示数据
- `POST /api/v2/demo/snapshot` - 创建数据快照

#### 配置

通过环境变量 `DEMO_MODE` 启用演示模式：
