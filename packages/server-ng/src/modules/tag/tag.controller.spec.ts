  ...
  describe('ts-rest Handlers', () => {
    describe('getTags()', () => {
      it('should return tags in ts-rest format', async () => {
        const mockTags = {
          items: [createMockTag({ id: 1, name: 'JavaScript', articleCount: 10 })],
          total: 1,
        };
        mockTagService.findAll.mockResolvedValue(mockTags);

        const handler = controller.getTags() as any;
    ...
    describe('createTag()', () => {
      it('should create a tag and return in ts-rest format', async () => {
        ...
        const handler = controller.createTag() as any;
      ...
      it('should handle null updatedAt gracefully', async () => {
        const createDto = createCreateTagDto({ name: 'Vue' });
        const mockCreatedTag = createMockTag({
          ...createDto,
          updatedAt: undefined,
        });
    ...
    describe('updateTag()', () => {
      it('should update a tag by name and return in ts-rest format', async () => {
        const updateDto = createUpdateTagDto({ name: 'JavaScript 2024' });
        const mockFoundTag = createMockTag({ id: 1, name: 'JavaScript' });
        const mockUpdatedTag = createMockTag({
          id: 1,
          ...updateDto,
          createdAt: '2024-01-01T00:00:00Z',
        ...
        const handler = controller.updateTag() as any;
        const result = await handler({
    ...
    describe('deleteTag()', () => {
      it('should delete a tag by name and return success', async () => {
        const mockFoundTag = createMockTag({ id: 1, name: 'ToDelete' });

        mockTagService.findByName.mockResolvedValue(mockFoundTag);
        mockTagService.remove.mockResolvedValue(undefined);

        const handler = controller.deleteTag() as any;
