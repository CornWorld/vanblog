...
export class MetricsController {
  ...
  getMetrics(): string {
    ...
    metrics.push(`http_requests_total ${stats.totalRequests}`);
    ...
    metrics.push(`http_requests_error_rate ${globalErrorRate}`);
    ...
    metrics.push(`http_requests_errors_total ${stats.errorRequests}`);
    ...
    metrics.push(`process_heap_bytes ${memoryUsage.heapUsed}`);
    ...
    metrics.push(`system_health_status ${healthStatus.status === 'healthy' ? 1 : 0}`);
    ...
    endpointErrorRates.forEach((endpoint) => {
      ...
      metrics.push(
        `http_requests_error_rate_by_endpoint{path="${endpoint.endpoint}"} ${endpoint.errorRate}`,
      );
    });
