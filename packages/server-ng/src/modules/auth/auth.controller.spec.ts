...
describe('AuthController', () => {
  ...
  describe('getLoginLogs', () => {
    it('should return login logs for admin user', async () => {
      const mockLogs = [
        {
          id: 1,
          username: 'testuser',
      ...
      mockLoginLogService.getLogs.mockResolvedValue(mockLogs);
      ...
      const result = await controller.getLoginLogs(req, query);

      expect(result).toEqual(mockLogs);
      expect(loginLogService.getLogs).toHaveBeenCalledWith({ username: 'testuser' });
    });
    ...
    it('should throw error for non-admin user', async () => {
      ...
      await expect(controller.getLoginLogs(req, query)).rejects.toThrow(HttpException);
    });
    ...
    it('should throw BadRequestException for invalid query', async () => {
      ...
      await expect(controller.getLoginLogs(req, invalidQuery)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('login_tsrest', () => {
    it('should login successfully via ts-rest', async () => {
      ...
      const handler = controller.login_tsrest();
    ...
    it('should return 401 for invalid credentials via ts-rest', async () => {
  ...
  describe('logout_tsrest', () => {
    it('should logout successfully via ts-rest', async () => {
      mockAuthService.revokeToken.mockResolvedValue(undefined);

      const handler = controller.logout_tsrest();
      const result = await handler({
        headers: {
    ...
    it('should handle logout without token via ts-rest', async () => {
      const handler = controller.logout_tsrest();
      const result = await handler({
        headers: {},
      } as any);
