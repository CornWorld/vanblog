...
const tagTest = test.extend<TagTestContext>({
  ...
  tagService: async ({ databaseMock, statisticsService, hookService }, use) => {
    const service = new TagService(databaseMock.db as any, statisticsService, hookService as any);
    await use(service);
  },
});
...
describe('TagService', () => {
  describe('findAll', () => {
    tagTest('should return paginated tags', async ({ tagService, databaseMock }) => {
      const mockTags = [
        {
          id: 1,
          name: 'Tag 1',
