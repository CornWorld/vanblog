...
interface BytemdAction {
  ...
  handler: {
    type: 'action';
    click: (ctx: BytemdEditorContext) => void;
  };
}
...
export function emoji(): BytemdPlugin {
  ...
  let lastSelection: { anchor: Position; head: Position } | null = null;
  ...
  const insertGlyph = (glyph: string, editor: BytemdEditorContext['editor']) => {
    ...
    actions: [
      {
        ...
        handler: {
          type: 'action',
          click: ({ root, editor }: BytemdEditorContext) => {
            lastSelection = editor.listSelections()?.[0] || null;
