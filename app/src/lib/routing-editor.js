...
export function initRoutingEditor({ TYPE_OPTIONS }) {
  ...
  async function refreshStatus() {
    let s;
    try { s = await pb.vanblog.routing.status(); } catch { return; }
    const banner = document.getElementById('health-banner');
    const unhealthy = s.caddyLastError || s.caddy_reachable === false;
    if (!unhealthy) { if (banner) banner.remove(); return; }
  ...
  async function refreshAudits() {
    let items;
    try { items = (await pb.vanblog.routing.audits()).items || []; } catch { return; }
    const panel = document.getElementById('audit-panel');
    if (!panel) return;
    const list = panel.querySelector('ul');
    if (!list) return;
    if (items.length === 0) { panel.remove(); return; }
    const summary = panel.querySelector('summary');
    if (summary) summary.textContent = `最近变更（${items.length} 条${items.length >= 10 ? '，仅显示最近 10 条' : ''}）`;
  ...
  function bindRow(tr) {
    ...
    headersEl?.addEventListener('blur', () => {
      ...
      if (!raw) return;
      try { JSON.parse(raw); headersEl.classList.add('border-green-500'); }
      catch (e) { headersEl.classList.add('border-red-500'); headersEl.title = 'Headers 不是合法 JSON：' + e.message; }
    });
    tr.querySelector('.btn-test').addEventListener('click', async () => {
      ...
      try {
        ...
        status.textContent = r.ok ? `✓ ${rule.id || '(新规则)'} 校验通过` : `✗ ${rule.id || '(新规则)'}：${r.error || '未知错误'}`;
      } catch (e) { status.textContent = '校验请求失败：' + e.message; }
    });
  }
  ...
  document.getElementById('btn-save').addEventListener('click', async () => {
    ...
    const rules = [];
    try { for (const tr of document.querySelectorAll('#rules-body tr')) rules.push(readRow(tr)); }
    ...
    try {
      ...
      else if (r.rolled_back) { status.textContent = '✗ 已回滚：' + (r.error || '未知错误'); alert('保存失败，已恢复到之前的规则：\n' + (r.error || '')); }
      else if (r.restart_needed) status.textContent = '已保存，重启 vanblog 后生效（或点"立即应用"）';
      else if (r.error) { status.textContent = '✗ 保存失败：' + r.error; alert('保存失败：' + r.error); }
      else status.textContent = '未应用：' + JSON.stringify(r);
    } catch (e) { status.textContent = '✗ 保存被拒绝：' + e.message; alert('保存被拒绝：\n' + e.message); }
    refreshStatus(); refreshAudits();
  });
  ...
  document.getElementById('btn-apply').addEventListener('click', async () => {
    ...
    try {
      ...
      else if (r.error) { status.textContent = '✗ 应用失败：' + r.error; alert('应用失败：' + r.error); }
      else status.textContent = '未应用：' + JSON.stringify(r);
    } catch (e) { status.textContent = '应用请求失败：' + e.message; alert('应用请求失败：' + e.message); }
    refreshStatus();
  });
  ...
  let renderCache = null, renderFetched = false;
  ...
  function renderSelectedLayer() {
    ...
    if (data == null) { renderOutput.textContent = layer === 'full' ? '（完整 config 不可用）' : '（无规则）'; return; }
    renderOutput.textContent = JSON.stringify(data, null, 2);
  }

  async function loadRender(force) {
    if (renderFetched && !force) { renderSelectedLayer(); return; }
    renderStatus.textContent = '加载中…'; renderError.classList.add('hidden');
    try {
      const r = await pb.vanblog.routing.render();
      renderCache = r; renderFetched = true; renderStatus.textContent = '';
      if (r.error) { renderError.textContent = '校验失败：' + r.error; renderError.classList.remove('hidden'); }
      renderOutput.textContent = (!r.userRoutes || r.userRoutes.length === 0) ? '（无用户规则，或规则校验未通过）' : '';
      if (r.userRoutes?.length) renderSelectedLayer();
    } catch (e) { renderStatus.textContent = ''; renderError.textContent = '加载失败：' + e.message; renderError.classList.remove('hidden'); }
  }

  let renderFirstOpen = true;
  renderPanel?.addEventListener('toggle', () => { if (renderPanel.open && renderFirstOpen) { renderFirstOpen = false; loadRender(true); } });
  document.getElementById('btn-render-refresh')?.addEventListener('click', () => loadRender(true));
  document.querySelectorAll('input[name="render-layer"]').forEach(el => el.addEventListener('change', renderSelectedLayer));
}
