...
declare module 'astro' {
  interface Locals {
    ...
    getSite(): Promise<Partial<SiteConfig> | null>;
  }
}
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (cookie) {
    try { client.authStore.loadFromCookie(cookie); } catch {}
  }
  ...
  context.locals.getSite = async () => {
    if (cachedSite && Date.now() - siteFetchTime < SITE_CACHE_TTL) return cachedSite;
    try {
