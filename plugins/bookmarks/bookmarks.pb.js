...
onBootstrap(function (e) {
    ...
    var collection = new Collection({
        ...
        fields: [
            ...
            { name: "owner", type: "relation", collectionId: usersCol.id, maxSelect: 1, required: true },
...
routerAdd("GET", "/api/bookmarks/list", function (e) {
    try {
        var records = $app.findRecordsByFilter("bookmarks", "id != ''", "-created", 100, 0);
        var items = [];
...
routerAdd("POST", "/api/bookmarks/create", function (e) {
    try {
        ...
        return e.json(200, { id: record.id, title: record.getString("title"), url: record.getString("url") });
    } catch (err) {
...
routerAdd("DELETE", "/api/bookmarks/{id}", function (e) {
    try {
        ...
        var record;
        try { record = $app.findRecordById("bookmarks", id); } catch (_) {
...
// Static assets
routerAdd("GET", "/plugins/bookmarks/{path...}", $vanblog.serveStatic("bookmarks"));
